...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mvDepth(frame.mvDepth), mvSemanticMask(frame.mvSemanticMask), mBowVec(frame.mBowVec), mFeatVec(frame.mFeatVec),
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imMask, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mvSemanticMask(NULL),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    // Frame ID
    ...
    //Semantic Mask
    mvSemanticMask = vector<int>(N,1);
    for(size_t i=0; i<mvKeys.size();i++)
    {
        const cv::KeyPoint &kp = mvKeys[i];
        const float &v = kp.pt.y;
        const float &u = kp.pt.x;
        const float ismask = imMask.at<char>(v,u);
        //std::cout << std::to_string(ismask) << std::endl;
        
        if (ismask > -0.5){
            mvSemanticMask[i] = 1;
            //std::cout << "Semantic Mask:"<< std::endl;
        }
        else {
    ...
    //std::cout << "Semantic Mask Gen" << std::endl;
