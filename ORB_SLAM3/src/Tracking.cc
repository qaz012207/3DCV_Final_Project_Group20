...
Sophus::SE3f Tracking::GrabImageRGBDWithMask(const cv::Mat &imRGB,const cv::Mat &imD,const cv::Mat &imM,const double &timestamp, string filename)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    cv::Mat imMask = imM;
    ...
    if (mSensor == System::RGBD){
        ...
        //std::cout << "Semantic Frame" << std::endl;
    }
    else if(mSensor == System::IMU_RGBD){
        mCurrentFrame = Frame(mImGray,imDepth,imMask,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    }
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(!pMP)
            ...
            bCreateNew = true;
        //Semantic Mask
        //if(mLastFrame.mvSemanticMask[i] == 0)
        //{
        //    bCreateNew = false;
        //}
    ...
    if(mSensor!=System::MONOCULAR && mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                //Semantic Mask
                //if(mCurrentFrame.mvSemanticMask[i] == 0)
                //{
                //    bCreateNew = false;
                    //std::cout << "Semantic Mask False" << std::endl;
                //}
