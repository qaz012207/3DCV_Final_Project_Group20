...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    //Semantic Mask
    //if(settings_ && settings_->isSemanticMask() == 1){
        //cv::Mat A = cv::Mat::zeros(cv::Size(imToFeed.cols, imToFeed.rows/2), CV_8UC1);
        //cv::Mat B = cv::Mat::ones(cv::Size(imToFeed.cols, imToFeed.rows/2), CV_8UC1);
        //cv::Mat H;
        //cv::vconcat(A, B, H);
        //std::cout << "col: " << std::to_string(H.cols) << "$\\pm$" << std::to_string(H.rows) << std::endl;
        //cv::Mat *imSemanticMask;
        //imSemanticMask = &H;
    //}
...
Sophus::SE3f System::TrackRGBDWithMask(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &semanticmask, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=RGBD  && mSensor!=IMU_RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    cv::Mat imToFeed = im.clone();
    cv::Mat imDepthToFeed = depthmap.clone();
    cv::Mat imSemanticMaskToFeed = semanticmask.clone();
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBDWithMask(imToFeed,imDepthToFeed,imSemanticMaskToFeed,timestamp,filename);
